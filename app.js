const APP_VERSION = 'FishLog V1.5 nav and layer fix';
...
let map, esriImagery, esriLabels, topoOverlay, topoVisible=false, labelsVisible=true;
...
function initMap(){
  ...
  topoOverlay = L.tileLayer(
    'https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png',
    { maxZoom: 17, opacity:0.34, attribution:'Topo © OpenTopoMap, SRTM, OpenStreetMap contributors' }
  );

  L.control.layers(
    { 'Esri Satellite': esriImagery },
    { 'Place Labels': esriLabels, 'Topo Overlay': topoOverlay },
...
function bind(){
  ...
  document.getElementById('layerBtn').onclick=()=>{
    if(!map || !topoOverlay) return;
    topoVisible = !topoVisible;
    if(topoVisible){ topoOverlay.addTo(map); document.getElementById('layerBtn').textContent='▰'; }
    else { map.removeLayer(topoOverlay); document.getElementById('layerBtn').textContent='▱'; }
