  ...
  groups.forEach(g=>{
    if(!expandedGroup && g.items.length>1){
      ...
      g.items.forEach(c=>{
        ...
        const marker = L.marker([Number(c.lat),Number(c.lng)], {icon:makeFishIcon(c, isBiggest), interactive:true}).addTo(catchLayer);
